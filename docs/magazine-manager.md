...
| 기간 | 2025년 10월 ~ 현재 |
| 인원 | 외주 단독 개발 (1인) |
| 상태 | 2026년 7월 실운영 이관 · 유지보수 중 |
| 역할 | 기획, 요구사항 분석, DB 설계, 백엔드/프론트엔드 개발, 서버 배포까지 전 과정 단독 수행 |
| GitHub | https://github.com/sunmok-labs |

## 사용 기술

`Python` · `FastAPI` · `SQLAlchemy` · `MySQL` · `Alembic` · `React` · `TypeScript` · `TailwindCSS` · `React Query` · `JWT`
...
**관련 트러블 슈팅** — 발송 그룹화 알고리즘 / 중복 발송 방지 필터
...
**관련 트러블 슈팅** — Cafe24 OAuth 2.0 연동 / Cafe24 배송 상태 자동 연동
...
- 개선 전 — 모든 상품이 한 번에 나열되어 스크롤이 길고 종류·월호가 뒤섞임 (`/magazine-manager/before_1.png`)
- 개선 전 — 선택 품목이 카드 형태, 구독기간 월 단위 입력, 소계 없음 (`/magazine-manager/before_2.png`)
- 개선 후 — 구독 종류 → 책 종류 단계별 필터, 테이블 행 형태, 구독기간 년 단위·판매가 자동 연동, 실시간 소계 (`/magazine-manager/after.png`)
...
> 구조 다이어그램: `subscription-period` — 그림의 글자는 데이터가 아니라 `components/diagrams/` 컴포넌트 안에 있다.

**구현**

1. 구독 개월 수(subscribe_period)를 독립 입력값으로 유지합니다. 실효 종료 호에서 역산이 불가능한 이유가 있습니다.
...
2. 모든 구독 기간 변경 작업이 단일 공식을 통해 재계산됩니다. 연장·단축 시 구독 개월 수를 먼저 갱신하고, 휴지는 별도 카운트로 항상 공식 위에 얹습니다.

   ```python
   def recalculate_final_end_issue(db, order_item):
...
> 구조 다이어그램: `send-grouping` — 그림의 글자는 데이터가 아니라 `components/diagrams/` 컴포넌트 안에 있다.

**구현**

1. 단품(group_monthly_for_send): 우편번호 + 수령 주소 기준 합포장. 수령인 정보가 충돌하면 이름 → 연락처 기준 가나다 정렬 후 첫 번째를 대표 수신인으로 자동 채택.

2. 정기구독(group_yearly_for_send): 주문 품목 1개 = 박스 1개. 실효 구독 범위·휴지 여부·중복 이력 등 다층 필터 적용.

3. VIP(group_vip_for_send): 우편번호 + 수령 주소 기준 합포장. 정기구독·단품 혼합 가능, 해외 주소 포함. 단품·정기구독 쿼리에서는 VIP 슬롯에 속한 주문을 자동 제외합니다.

   ```python
   def exclude_active_vip(query, active_slots, address_ref=Address):
       if not active_slots:
           return query
...
> 구조 다이어그램: `duplicate-prevention` — 그림의 글자는 데이터가 아니라 `components/diagrams/` 컴포넌트 안에 있다.

**구현**

1. 단품 필터: 호 비교 없이 전체 발송 이력 기준으로 이미 처리된 품목을 제외합니다.

   ```python
   already_sent_ids = {row[0] for row in
...
2. 정기구독 추가 조건: 해당 호 이력 확인 + 구독 범위 내 + 휴지 없음 세 조건을 모두 만족해야 발송 대상으로 선별합니다.
...
3. 순차 발송 위반 검사(find_sequence_violations): 발송 전 중간 호가 누락된 채 건너뛰는 경우를 사전에 감지해 관리자에게 알립니다. 구독 시작 호부터 목표 호 직전까지 각 호를 순회해 발송 이력과 휴지 모두 없는 호를 누락 호로 판정합니다.

**결과**

단품·정기구독·VIP 3종 발송에서 중복 발송 0건을 유지하며 운영 중입니다. 순차 위반 사전 알림으로 구독자에게 잡지가 중간 호를 건너뛰어 배송되는 사고를 방지합니다.

### 04. Cafe24 OAuth 2.0 연동
...
1. 인증 시작: 단순 URL 생성이 아닌, CSRF 방지를 위해 state 파라미터를 서버에서 서명합니다. nonce로 재사용을 방지하고 timestamp로 TTL을 검증합니다.
...
2. 콜백 처리: Cafe24가 리다이렉트한 콜백에서 state 유효성을 3단계로 검증합니다.
...
3. 토큰 자동 갱신: API 호출 레이어에서 요청 전 access_token 만료 여부를 확인해 자동으로 갱신합니다. 갱신 엔드포인트를 별도로 제공해 프론트엔드에서도 수동 갱신이 가능합니다.
...
1. customers 테이블에 복합 Unique 제약을 추가했습니다.
...
2. 고객을 찾거나 생성하는 모든 경로를 전화번호 단독 조회에서 전화번호 + 이름 복합 조회로 변경했습니다. Cafe24 주문 import, 레거시 엑셀 이관 스크립트, 고객 검색 API가 모두 여기에 해당합니다.

3. 운영 DB 마이그레이션: 이미 저장된 데이터에 동일 번호를 공유하는 고객 레코드가 있을 수 있었습니다. Alembic 마이그레이션 실행 전 중복 데이터 유무를 확인하고 수동으로 정리한 뒤 제약 조건을 추가했습니다.

**결과**

이름과 번호가 다른 조합으로 주문이 들어와도 각자 독립된 고객 레코드에 정확하게 귀속됩니다. 전화 상담 시 고객이 주문 당시 입력한 이름과 번호 조합으로 즉시 조회할 수 있습니다.

### 06. Cafe24 배송 상태 자동 연동
...
> 구조 다이어그램: `cafe24-sync` — 그림의 글자는 데이터가 아니라 `components/diagrams/` 컴포넌트 안에 있다.

**구현**

1. import 직후 정기구독 standby 등록: 같은 주문에 정기구독 품목이 여러 개 있어도 order_item_code 배열로 묶어 POST 1회로 처리합니다. Cafe24는 request 항목당 shipping_code 1개를 발급하므로 해당 주문의 모든 정기구독 품목에 동일한 shipping_code를 저장합니다. 미리보기(dry_run) 모드이면 Cafe24 POST를 완전히 건너뜁니다. POST 실패 시 ImportSkip 예외 → 해당 주문 DB rollback + import 실패 처리.
...
2. 발송 완료 시 원자성 보장: DB flush(내부 상태 변경) → Cafe24 API 호출 → 성공 시 commit / 실패 시 rollback 순서로 처리합니다.
...
3. 공식 문서 vs 실제 동작 불일치: Cafe24 공식 문서 파라미터 표에 order_id가 Optional로 기재되어 있어 처음에는 생략했습니다. 실제 운영 환경에서 PUT 요청이 모두 502로 실패해 확인한 결과, order_id 누락 시 Cafe24가 422를 반환함을 발견했습니다.

   ```python
   # 수정 전 — order_id 누락 → Cafe24 422 → 우리 시스템 502
...
> 구조 다이어그램: `pause-design` — 그림의 글자는 데이터가 아니라 `components/diagrams/` 컴포넌트 안에 있다.

**구현**

1. pauses 테이블 설계. pause_issue는 생성 후 절대 변경하지 않는 불변 필드로 설정합니다. 수정이 필요하면 소프트 삭제 후 재생성합니다.
...
2. 발송 로직 수정. 발송 대상 선별 쿼리에 해당 호의 활성 pause 여부를 NOT EXISTS 조건으로 추가합니다. 발송하려는 월이 pause에 포함되면 자동으로 제외됩니다.
...
3. SQLAlchemy 구현. pause_issue == 이번 발송 호 단순 등호 비교로 처리되므로 필터가 간결하게 유지됩니다.
...
4. final_subscribe_end_issue 재계산. pause 생성 시 +1개월, 소프트 삭제 시 −1개월. 기존 구독 기간 공식(연장·단축·기간이동과 동일한 경로)에 활성 pause 개수를 얹어 처리하므로 누적 오차가 없습니다.
