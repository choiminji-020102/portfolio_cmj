    ...
    aiFeatures: [
      {
        ...
        troubles: [
          {
            title: "배포 환경 벡터 DB 생성 실패 & 재로딩 성능 저하",
            stars: 5,
            problem:
              "로컬에선 되던 RAG가 배포 서버에서 Chroma DB 생성에 실패했고, DB가 있어도 질문마다 벡터 DB를 통째로 다시 로딩해 응답이 느렸습니다.",
            solution:
              "persist 경로를 절대경로 /RAG/chroma → 프로젝트 기준 상대경로로 바꿔 이식성을 확보하고, 함수 진입마다 캐시를 초기화하던 _vectordb=None 을 제거해 인메모리 캐시를 복원했습니다. 경로 없음·빈 문서 예외 방어도 추가했습니다.",
            effect:
              "로컬·배포가 동일 경로로 동작하고, 첫 요청 이후 재로딩이 사라져 응답 지연과 임베딩 비용이 줄었습니다.",
            tags: ["배포환경", "성능", "캐시"],
          },
          {
            title: "질문 라우팅 아키텍처 설계 — 4-way 분류",
            stars: 5,
            problem:
              "모든 질문을 한 경로로 처리하다 보니, 무관한 질문(‘로또 번호 알려줘’)도 성실히 답해 챗봇 정체성이 흐려지고, 개체 조회(‘103번 소 상태’)와 일반 지식(‘발정 시기’)이 뒤섞여 답변 품질이 들쭉날쭉했습니다.",
            solution:
              "classifier 노드가 질문 의도를 4종(rag·cow_info·general·irrelevant)으로 먼저 분류하고, LangGraph 조건부 분기로 전용 노드에 라우팅했습니다. LLM이 엉뚱한 라벨을 뱉어도 irrelevant로 떨어지는 방어 코드를 넣었습니다.",
            effect:
              "무관 질문을 LLM 호출 전에 차단해 토큰 낭비를 없애고, 의도별 최적 데이터 소스(사내 문서·농장 DB·대화 맥락)를 연결해 답변 품질을 안정화했습니다.",
            tags: ["LangGraph", "아키텍처", "의도분류"],
            diagram: "route",
          },
          {
            ...
            stars: 5,
            problem:
              "사내 문서를 검색해 답하는 RAG에서, 문서에 관련 내용이 없는 no-hit 케이스가 설계에 없어 답변이 비거나 흐름이 끊겼습니다.",
            solution:
              "‘프롬프트 내부 처리 vs 노드 분기’를 응답 속도·유지보수·의도 제어·구현 난이도로 비교했습니다. no-hit는 별도 노드를 만들 만큼 복잡하지 않다고 판단해, 프롬프트에 ‘자료가 없으면 알고 있는 지식으로 답하라’는 조건부 지시를 넣는 fallback을 택했습니다.",
            effect:
              "검색 실패에도 끊김 없이 응답하고, 그래프를 단순하게 유지했습니다. similarity_search_with_score로 유사도 점수를 함께 실어 근거의 투명성도 높였습니다.",
          ...
          },
          {
            ...
            stars: 4,
            problem:
              "여러 사용자가 동시에, 한 사용자가 여러 채팅방을 오가며 쓰는데 대화 맥락이 섞이면 안 됐습니다. 초기엔 기록을 dict로 누적했는데, 프롬프트에 직렬화하면 LLM이 대화 흐름으로 인식하기 어려웠습니다.",
            solution:
              "(user_id, chat_id) 튜플을 키로 세션을 격리하고, .get(key, [])로 신규 세션을 KeyError 없이 빈 리스트로 시작했습니다. 저장 포맷은 dict 대신 ‘사용자: …’, ‘소담이: …’ 화자 접두 문자열 리스트로 바꿨습니다.",
            effect:
              "유저·채팅방별 맥락이 완전히 격리되고, 대화 스크립트형 포맷 덕에 ‘그 소·아까 그거’ 같은 멀티턴 후속 질문의 맥락도 자연스럽게 이어졌습니다.",
          ...
          },
          {
            ...
            stars: 4,
            problem:
              "RAG 답변에 근거 출처를 붙이자 자잘한 오류가 연쇄로 터졌습니다. 파일 확장자가 그대로 노출되고, 같은 문서가 중복 표기되고, ** 같은 마크다운 기호가 날것으로 나가고, 영어 질문에도 한글 ‘출처:’가 붙었습니다.",
            solution:
              "문서 임베딩 시점부터 확장자를 제거한 파일명만 metadata에 저장하고, 출처는 중복 제거 후 쉼표로 구분, 특수기호 제거, 질문 언어에 맞춰 [출처: …] / [Source: …]로 분기하도록 프롬프트 규칙과 파이프라인을 다듬었습니다. (커밋 5건에 걸쳐 반복 개선)",
            effect:
              "출처가 깔끔한 파일명으로, 중복·기호 없이, 질문 언어에 맞춰 표기돼 답변의 신뢰성과 가독성이 함께 올라갔습니다.",
            tags: ["RAG", "응답품질", "다국어"],
          },
        ],
      },
