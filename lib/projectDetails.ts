...
if len(raw_documents) == 0:
  ...
  /*
    삼성메디슨 인턴 — 파트 1 (2024.03~04, 다중 구조물 검출).
    인턴 기간 중 사수가 바뀌며 담당 과제가 둘로 나뉜다. 파트 2(2024.05~08, AddCaliper)는
    자료가 오면 별도 slug 로 추가하고, 그때 이 slug 를 samsung-medison-detection 으로 정리한다.

    서술 범위는 의도적으로 보수적이다. 사내 의사결정(과제 진행 여부), 협력사 평가,
    데이터 규모, 미출시 제품의 적용 질환·구조물 명은 싣지 않는다.
    본인이 구현한 방식, 공개 모델에 대한 판단 근거, 직접 측정한 값만 남긴다.
  */
  {
    ...
    teamSize: "AI Vision 그룹 · 단독 수행",
    summary:
      "하복부 초음파 영상에서 6종 구조물을 검출하는 과제를 맡았습니다. 학습을 반복하며 필요한 도구를 직접 만들었고 — 결과를 눈으로 확인하는 시각화 도구, 어노테이션을 학습 포맷으로 바꾸는 GT 생성 함수, 영상에서 데이터셋까지의 처리 파이프라인 — 성능이 정체되자 원인을 추적해 병목이 모델이 아니라 정답 데이터에 있음을 규명했습니다.",
    stack: [
      "Python",
      "PyTorch",
      "YOLOv8",
    ...
    backgroundLabel: "과제 배경",
    background:
      "하복부 초음파 영상에서 6종 구조물을 동시에 검출하는 과제였습니다. 학습을 돌릴 수는 있었지만, 넣을 정답 레이블도 나온 결과가 맞는지 확인할 수단도 없는 상태에서 시작했습니다.",
    backgroundEffect:
      "두 달 동안 실제로 한 일은 모델을 학습시킨 것이 아니라, 학습이 가능한 상태를 만드는 일이었습니다.",
    backgroundGoal:
      "실시간 판독 화면에 올릴 수 있는 다중 구조물 검출 모델을 찾고, 그 모델을 학습시킬 데이터셋을 확보하는 것.",
    myRole:
      "데이터 처리부터 학습, 결과 검증, 원인 규명까지 단독으로 수행했습니다. 원격 서버에 학습 환경을 직접 구축하고, 조건을 바꿔가며 학습을 반복하면서 그때그때 막힌 지점을 풀기 위한 도구를 만들었습니다. 사수와는 영어로 협업했습니다.",
    myRoleAreas: [
      {
        title: "학습 가능한 상태 만들기",
        desc: "영상에서 데이터셋까지의 처리 파이프라인, CVAT 어노테이션을 학습 포맷으로 바꾸는 GT 생성, 각 단계 결과를 눈으로 확인하는 검증 도구를 구현",
      },
      {
        title: "모델 선정 · 반복 학습",
        desc: "배포 제약을 기준으로 Segmentation 후보 모델을 비교해 선정하고, 전처리·옵티마이저·Augmentation 조건을 바꿔가며 학습을 반복",
      },
      {
        title: "병목 규명 · 보고",
        desc: "성능 정체의 원인을 추적해 정답 데이터 품질 문제로 좁히고, 임상의 검증을 거쳐 데이터 재구축이 선행되어야 함을 보고",
      },
    ],
    myRolePipeline: [
      "영상 프레임 추출 · 크롭",
    ...
    aiFeatures: [
      {
        name: "학습 가능한 상태 만들기",
        tagline:
          "모델을 돌리기 전에, 넣을 데이터와 결과를 확인할 수단이 먼저 필요했습니다.",
        metric: "검증 도구 3종 · GT 생성 · 처리 파이프라인",
        problemLabel: "확인할 수 없던 것들",
        problemList: [
          "정규화된 레이블 좌표가 실제 이미지 위 어디에 찍히는지 숫자만으로는 알 수 없다",
          "예측이 어디서 틀렸는지 — 위치인지 클래스인지 — 지표 하나로는 구분되지 않는다",
          "좌표계를 변환할 때마다 결과가 맞는지 검산할 방법이 없었다",
        ],
        solutionLabel: "직접 만든 도구",
        solutionBlocks: [
          {
            title: "Bounding box 렌더링",
            points: [
              "정규화 레이블을 픽셀 좌표로 역산해 이미지 위에 직접 그림",
              "폴더 경로만 넘기면 전체 이미지가 자동 처리되도록 구성",
            ],
          },
          {
            title: "GT · 예측 비교",
            points: [
              "정답 박스를 그린 이미지와 모델 예측 결과를 가로로 병합해 나란히 비교",
              "박스·클래스명·파일명을 함께 표시해 어긋난 지점을 바로 짚을 수 있게 함",
            ],
          },
          {
            title: "Mask 렌더링",
            points: [
              "Segmentation 전환 이후 폴리곤 좌표를 선으로 그려 표시",
              "변환한 레이블이 옳은 좌표를 따왔는지 검산하는 데도 같은 함수를 사용",
            ],
          },
        ],
        troublesLabel: "이 도구들이 잡아낸 것",
        troubles: [
          {
            title: "증상만 보고는 원인을 좁힐 수 없었다",
            problem:
              "검출 위치는 정확한데 클래스 이름이 다르게 표시되고, 마스크 모양은 정확한데 색상이 다르게 렌더링됐다. 두 경우 모두 눈에 보이는 증상과 실제 원인이 달랐다.",
            solution:
              "학습 산출물(labels.jpg)을 역추적해 각각 어노테이션과 설정 파일의 클래스 인덱스 정의 불일치, 어노테이션의 id 값이 실제 라벨링 순서와 다르게 기록된 문제임을 확인했다.",
            effect:
              "이후 각 단계의 결과를 시각적으로 확인한 뒤 다음으로 넘어가는 절차를 고정했다.",
            tags: ["디버깅", "클래스 인덱스"],
            tech: ["YOLOv8", "YAML", "CVAT"],
            details: [
              {
                heading: "증상과 원인",
                blocks: [
                  {
                    ...
                    rows: [
                      [
                        ...
                        "어노테이션 파일과 설정 파일의 클래스 인덱스 정의가 서로 달랐다",
                      ],
                      [
                        "마스크 모양은 정확한데 색상이 다르게 렌더링",
                        "어노테이션의 id 값이 실제 라벨링 순서와 다르게 기록돼 있었다",
                      ],
                    ],
                  },
                  {
                    type: "text",
                    text: "둘 다 증상만 보고는 원인을 좁힐 수 없었습니다. 검증 도구가 없었다면 학습 결과가 낮은 이유로 뭉뚱그려졌을 문제입니다.",
                  },
                ],
              },
              {
                heading: "클래스 정의에서 어긋나는 지점",
                blocks: [
                  {
                    type: "text",
                    text: "데이터셋 설정에서 names 를 리스트로 주면 나열 순서대로 0부터 자동 매칭됩니다. 이러면 실제 라벨링 인덱스와 어긋날 수 있어, dict 형태로 인덱스를 명시했습니다.",
                  },
                  {
                    type: "code",
                    code: `# 리스트 → 나열 순서대로 0부터 자동 매칭 (실제 인덱스와 어긋날 수 있음)
names: [struct_a, struct_b, ...]

# dict → 인덱스를 명시. 반드시 0부터 시작해야 한다
names:
  0: struct_a
  1: struct_b
  ...`,
                  },
                  {
                    type: "list",
                    items: [
                      "인덱스를 1부터 시작하면 nc(클래스 수) 불일치 오류가 난다",
...
우하단 = 좌상단 + (BW, BH)`,
          ...
          {
            title: "어노테이션을 그대로 학습 포맷으로 쓸 수 없었다",
            problem:
              "제공받은 이미지는 이미 크롭된 상태인데 마스크 좌표는 원본 해상도 기준으로 기록돼 있었다. 게다가 어노테이션 도구가 미검출 구조물을 프레임 밖 좌표로 기록해, 정규화하면 유효 범위(0~1)를 벗어났다.",
            solution:
              "크롭은 종횡비 변경이 아니라 여백 제거이므로 스케일링이 아닌 shift 로 처리해 좌표계를 맞췄다. 프레임 밖 좌표는 XML 단계에서 선별하기 어려워, XML 파싱을 포기하고 도구가 클래스별로 출력하는 annotation txt 를 병합하는 방식으로 전환했다.",
            effect:
              "Segmentation 학습에 바로 넣을 수 있는 GT 레이블 데이터셋을 확보했고, 생성 결과는 mask 렌더링 함수로 확인한 뒤 학습에 투입했다.",
            tags: ["좌표계 정합", "GT 생성"],
            tech: ["Python", "CVAT", "XML"],
            details: [
              {
                heading: "좌표계 정합 — 중앙 크롭이 아니었다",
                blocks: [
                  {
                    type: "text",
                    text: "먼저 중앙 크롭을 가정해 계산했는데 렌더링 결과가 어긋났습니다. 실제로는 가로 제거량이 좌우 비대칭이었습니다.",
                  },
                  {
                    type: "code",
                    code: `가로 제거량  →  좌 200 / 우 180   (비대칭)
세로 제거량  →  상  60 / 하  60

픽셀 shift :  x' = x − 200        y' = y − 60
정규화     :  nX = x' / CW        nY = y' / CH`,
                  },
                  {
                    type: "text",
                    text: "크롭은 종횡비를 바꾸는 조작이 아니라 여백을 잘라내는 조작이므로, 스케일링이 아니라 shift 로 처리해야 한다는 점을 이때 확인했습니다.",
                  },
                ],
              },
              {
                heading: "방식 A — XML 파싱에서 드러난 한계",
                blocks: [
                  {
                    type: "text",
                    text: "XML 트리를 파싱해 클래스별 폴리곤 좌표를 추출하고, 크롭 좌표계로 변환한 뒤 정규화해 기록하는 방식을 먼저 만들었습니다. 구현하며 규명한 이슈는 아래와 같습니다.",
                  },
                  {
                    type: "table",
                    head: ["이슈", "대응"],
                    rows: [
                      ["좌표 기준 불일치 (원본 vs 크롭본)", "shift 후 재정규화"],
                      [
                        "name 태그가 지정 클래스명이 아닌 작업물 명으로 기록",
                        "정규식 패턴 치환",
                      ],
                      [
                        "id 값이 실제 라벨링 인덱스와 불일치",
                        "label 태그 순서에서 인덱스를 역참조",
                      ],
                      ["객체 구분 누락", "id 가 바뀔 때마다 줄바꿈"],
                    ],
                  },
                  {
                    type: "text",
                    text: "그런데 정규화 결과가 1을 넘는 케이스가 나왔습니다. 어노테이션 도구가 미검출 구조물을 프레임 밖 좌표로 기록하고 outside 태그로만 표시하는데, 이를 XML 단계에서 걸러내기 어려웠습니다.",
                  },
                ],
              },
              {
                heading: "방식 B — 클래스별 txt 병합으로 전환",
                blocks: [
                  {
                    type: "text",
                    text: "XML 파싱 자체를 포기하고, 도구가 클래스별로 개별 출력하는 annotation txt 를 병합해 최종 레이블을 만드는 방식으로 바꿨습니다.",
                  },
                  {
                    type: "code",
                    code: `class_0.txt ┐
class_1.txt ├→  merge_separate_annote  →  frame별 통합 label txt
    ⋮       ┘`,
                  ...
                  {
                    type: "text",
                    text: "구조적으로 막히는 지점을 우회하는 대신 입력 자체를 바꾼 경우입니다. 별도로 단일 클래스만 분리한 데이터셋도 구성했는데, 이때는 클래스 id 를 0으로 통일하고 객체가 없는 이미지도 빈 txt 로 유지해야 학습이 성립했습니다.",
                  },
                ],
              },
            ],
          ...
          {
            title: "프레임마다 유효 영상 영역의 크기가 달랐다",
            problem:
              "원본 영상에서 프레임을 추출해 크롭하는데, 같은 영상 안에서도 프레임별로 유효 영역 크기가 달랐다. 프레임별로 개별 크롭하면 데이터 크기가 제각각이 되고, 고정 크기로 자르면 일부 프레임의 유효 영역이 잘려나간다.",
            solution:
              "contour 를 추출해 프레임마다 유효 영역의 사각형 좌표를 구한 뒤, 그중 최빈값을 그 영상의 대표 크롭 크기로 채택해 전 프레임에 동일하게 적용했다.",
            effect:
              "영상 내부의 크기 일관성을 유지하면서 영상 간 편차는 개별 대응할 수 있게 됐다. 영상에서 학습 데이터까지의 처리를 하나의 파이프라인으로 묶었다.",
            tags: ["데이터 파이프라인", "OpenCV"],
            tech: ["Python", "OpenCV"],
            details: [
              {
                heading: "처리 흐름",
                blocks: [
                  ...
                  },
                  {
                    type: "list",
                    items: [
                      "영상 앞뒤의 무효 구간을 제외하고 유효 프레임 구간만 추출",
                      "파일명을 0 패딩 형식으로 강제해 문자열 정렬 오류를 방지",
                      "프레임별 사각형 좌표는 txt 로 listing, 크롭 결과는 표로 정리",
                    ],
                  },
                ],
              },
...
5. 해당 영상의 전 프레임을 동일 크기로 crop`,
                  },
                  {
                    type: "text",
                    text: "프레임 단위가 아니라 영상 단위로 대표값을 정하는 방식입니다. 프레임별 노이즈에는 흔들리지 않으면서, 영상마다 다른 촬영 조건에는 각각 대응할 수 있습니다.",
                  },
                ],
              },
            ],
      ...
      {
        name: "모델 선정 — 배포 제약에서 역산",
        tagline:
          "성능이 아니라, 장비에 올라갈 수 있는지를 기준으로 골랐습니다.",
        metric: "실측 지연 CPU ONNX 86ms vs TensorRT 1.2ms",
        ...
        solutionBlocks: [
          {
            ...
            points: [
              "CT 학습 모델은 영상 특성이 달라 배제 — CT 는 영상이 선명한 반면 초음파는 노이즈가 지배적이라 같은 의료영상이어도 전이 이점이 없다",
              "초음파 Segmentation 을 직접 다뤘어도 FPS·연산속도가 공개되지 않은 모델은 탑재 가능 여부를 판단할 수 없어 보류",
            ],
          },
          {
            title: "태스크 요건으로 2차 배제",
            points: [
              "단일 GPU 83 FPS 로 실시간이 가능한 모델도, 단일 클래스만 지원하면 다중 구조물 과제에는 쓸 수 없다",
              "bbox 와 segmentation 을 함께 내고 기존 학습 환경을 재사용할 수 있는 쪽으로 좁힘",
            ],
          },
          {
            title: "실측으로 최종 판단",
            points: [
              "논문 수치가 아니라 실제 배포 조건에서 추론 지연을 직접 측정",
              "같은 모델이 배포 환경에 따라 수십 배 차이 — 이 값을 기준으로 채택",
            ],
          },
        ],
        troublesLabel: "여기서 알게 된 것",
        troubles: [
          {
            title: "성능·속도를 통과해도 제품에 못 들어가는 조건이 있었다",
            problem:
              "정확도와 추론 속도를 기준으로 모델을 골라 학습까지 진행했는데, 채택한 모델의 라이선스가 AGPL-3.0이었다.",
            solution:
              "상용 제품에 탑재하려면 소스 공개 의무가 발생하거나 별도의 상용 라이선스 취득이 필요하다는 점을 확인해, 성능·속도 요건과 무관하게 그대로는 탑재가 불가하다는 사실을 함께 정리해 보고했다.",
            effect:
              "속도와 정확도로 모델을 고르는 데까지는 갔지만, 제품에 들어가려면 기술 지표 바깥의 조건이 먼저 통과되어야 한다는 것을 알게 됐다.",
            tags: ["라이선스", "제품 제약"],
            details: [
              {
                heading: "후보 모델 비교",
                blocks: [
                  {
                    ...
                    rows: [
                      [
                        "CT 기반 Segmentation 계열",
                        "배제",
                        "CT 학습 모델 — 초음파는 노이즈가 지배적이라 전이 이점 없음",
                      ],
                      [
                        "Transformer + U-Net 계열",
                        "보류",
                      ...
                      [
                        "real-time instance segmentation 계열",
                        "후보",
                        "실시간 처리 가능, FPS 확보",
                      ],
                      [
                        "YOLOv8-seg",
                        "채택",
                        "bbox + segmentation 동시 지원, 기존 학습 환경 재사용 가능",
                      ],
                    ],
                  },
                ],
              },
              {
                ...
                blocks: [
                  {
                    type: "text",
                    text: "배포 환경별로 직접 측정한 값입니다 (batch 8~10 구간).",
                  },
                  {
                    type: "code",
...
TensorRT  :  1.21 / 1.47 / 2.18 / 2.79 / 4.02    ms`,
                  ...
                  {
                    type: "text",
                    text: "같은 모델인데 배포 환경에 따라 지연 시간이 수십 배 차이 났습니다. 논문에 적힌 FPS 하나로 탑재 가능 여부를 판단할 수 없다는 뜻이었습니다.",
                  },
                ],
              },
            ],
      ...
      {
        name: "병목의 재정의 — 모델이 아니라 정답 데이터",
        tagline:
          "지표가 오르지 않을 때 학습을 한 번 더 돌리는 대신, 어디가 막혔는지를 물었습니다.",
        metric: "조정과 결과 사이에 상관 없음 → 기준 쪽을 의심",
        problemLabel: "관측된 것",
        problemList: [
          "여러 run 에 걸쳐 전체 mAP 가 좁은 구간을 벗어나지 못했다",
          "클래스별 등락 방향이 run 마다 뒤집혔다 — 한 클래스가 0.127에서 0.439로 오르는 동안 다른 클래스는 0으로 붕괴했다",
          "전처리·옵티마이저·learning rate·freeze layer·Augmentation 을 바꾸면 수치는 움직였지만 방향이 일정하지 않았다",
        ],
        solutionLabel: "추론과 검증",
        solutionBlocks: [
          {
          ...
          {
            title: "검증 — 판단에 필요한 지식을 가진 사람을 찾아감",
            points: [
              "구조물 경계가 옳게 그려졌는지는 임상 지식 없이 판단할 수 없었다",
          ...
          },
          {
            ...
            points: [
              "GT 마스크가 실제 구조물 경계보다 넓게 그려져 있음을 확인",
              "이 상태로는 모델을 어떻게 조정해도 학습이 성립하지 않는다는 판단을 받음",
              "데이터 재구축이 선행되어야 한다는 결론을 정리해 보고",
            ],
          },
        ],
        troublesLabel: "근거로 쓴 학습 결과",
        troubles: [
          {
            title: "수치는 움직이는데 방향이 없었다",
            problem:
              "조건을 바꿔 학습을 반복했지만 전체 mAP 는 좁은 구간에 머물렀고, 클래스별 등락은 run 마다 방향이 뒤집혔다.",
            solution:
              "run 별 지표를 정리해 조정과 결과 사이에 설명 가능한 상관이 없다는 것을 확인하고, 이를 정답 데이터 가설의 근거로 삼았다.",
            effect:
              "'성능이 안 나온다'가 아니라 '조정이 반영되지 않는다'로 문제를 다시 정의할 수 있었다.",
            tags: ["실험 기록", "원인 규명"],
            details: [
              {
                ...
                blocks: [
                  {
                    type: "table",
                    head: ["run", "조건", "전체 mAP50"],
                    rows: [
                      ["01", "초기 학습", "0.245"],
                      ["02", "조건 변경 후 3회 반복", "0.167 → 0.146 → 0.140"],
                      ["03", "조건 변경 후 2회 반복", "0.151 → 0.164"],
                      ["04", "조건 변경", "0.149"],
                    ],
                  },
                  {
                    type: "text",
                    text: "전체 mAP 는 여러 run 에 걸쳐 0.14~0.25 구간을 벗어나지 않았습니다. 같은 학습 안에서 한 클래스가 0.127에서 0.439로 급등하는 동안 다른 클래스는 0으로 떨어지는 식으로, 클래스별 등락 방향도 run 마다 뒤집혔습니다.",
                  },
                ],
              },
              {
                ...
                blocks: [
                  {
                    type: "list",
                    items: [
                      "전처리 — sharpen 적용 데이터셋을 따로 만들어 원본과 비교 학습, Mean/Median/Gaussian filter 와 Thresholding 검토",
                      "가중치 — 모델 크기별 사전학습 가중치 비교, 백본 후보로 EfficientNet 별도 학습",
                      "optimizer — Adam / AdamW / SGD / NAdam / RAdam / RMSProp",
                      "learning rate — 0.001 / 0.005 / 0.01, freeze layer 1 / 3 / 10",
                      "augmentation — U-Net 논문 기준(shift·rotate·zoom·flip)과 gaussian noise 조합",
                    ],
                  },
                  {
                    type: "text",
                    text: "조건을 하나씩 바꿔가며 비교했지만, 어떤 축을 움직여도 결과가 같은 방향으로 반응하지 않았습니다. 이 점이 모델이 아닌 데이터를 의심하게 된 출발점이었습니다.",
                  },
                ],
              },
            ],
    ...
    featuresLabel: "그 외 수행",
    features: [
      "원격 서버에 CUDA · conda · 가상환경 · PyTorch 학습 환경을 직접 구축",
      "Object Detection 으로 시작해 Instance Segmentation 으로 태스크 확장",
      "6종 다중 클래스 학습과 단일 클래스 학습을 별도 데이터셋으로 구성해 비교",
      "학습 산출물과 크롭 결과를 표로 정리해 조건별 비교가 가능한 형태로 기록",
    ],
    closingLabel: "마무리",
    closing: [
    ...
    writeups: [
      {
        title: "[YOLOv8] .yaml 파일 클래스 구성 — 인덱스 매칭 오류의 원인·증상·검증법",
      ...
      {
        title: "XML 파일을 txt 로 변환 — GT 레이블 생성",
        href: "https://blog.naver.com/t3335150/223416017978",
      },
      {
        title: "단일 클래스 Segmentation 학습",
