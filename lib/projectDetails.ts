...
export const lightProjects: LightProject[] = [
  {
    ...
    troubles: [
      {
        title: "챗봇 '소담이' — 기존 GPT의 한계를 RAG로 넘어서다",
        body:
          "범용 GPT는 낙농 도메인에서 환각(없는 정보를 지어냄)·최신 정보 미반영·전문 용어 혼동 문제가 있었습니다. 이를 해결하기 위해 낙농 전문 문서를 임베딩해 Vector DB에 저장하고, 질문과 관련된 문서를 먼저 검색해 그 내용을 근거로 답변하는 RAG 구조를 적용했습니다. 답변에는 항상 출처를 함께 표시해 신뢰도를 높였습니다.",
      },
      {
        title: "Query Routing — 질문을 4가지 유형으로 자동 분류",
        body:
          "LangChain 기반 Agent로 사용자 질문을 '낙농 전문 지식 / 농장 기록 조회 / 일반 대화 / 무관 질문' 4가지 유형으로 자동 분류(Query Routing)했습니다. 전문 지식은 RAG 검색으로, 농장 기록 조회는 사용자 데이터 조회로 각각 경로를 분기해 상황별 응답을 최적화했습니다. 또한 Prompt Engineering으로 GPT에 '낙농 상담사' 역할을 부여하고, 사용자의 연령대와 상황에 따라 설명 방식을 자동으로 조절했습니다.",
      },
      {
        title: "이어서 상담하는 1:1 대화 UI",
        body:
          "과거 대화 히스토리를 기반으로 이어서 상담할 수 있도록 '채팅 목록 + 대화창' 이중 패널 구조로 설계했습니다. 앱 전역 어디서나 즉시 질문할 수 있는 떠다니는 챗봇 버튼을 두고, 버튼 위치는 사용자가 자유롭게 조정할 수 있게 했습니다.",
      },
    ],
  },
];
