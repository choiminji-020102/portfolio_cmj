...
export interface LightProject {
  ...
  overviewFigures?: {
    ...
    /** 이미지에 라벨된 구조물 */
    caption?: string;
    /** caption 이 이미지 라벨이 아니라 과제의 결론·목표를 진술하는 문장일 때 —
        본문 크기로 키운다. 나머지 캡션(구조물 이름 등)은 작은 라벨 그대로 둔다 */
...
if len(raw_documents) == 0:
  ...
  /*
    ...
    인턴 기간 중 사수가 바뀌며 담당 과제가 둘로 나뉜다. 파트 2(2024.05~08, 부인과
    초음파 난포 검출 — 2D Follicle™)는 아래 samsung-medison-detection slug 로 있다.
...
실제: 가로 제거량이 비대칭 (좌 200 / 우 180)`,
  ...
  /*
    삼성메디슨 인턴 — 파트 2 (2024.05~08, 부인과 초음파 난포 검출 · 2D Follicle™ 팀).
    파트 1 과제 중단 후 다른 사수의 팀으로 옮겨 맡은 과제. 본인이 제공한
    포트폴리오-삼성메디슨-part2 원고를 유일한 출처로 삼는다.

    이미지 6장은 원고에 첨부된 PNG(base64)를 public/samsung-medison-detection/ 에
    그대로 추출한 것. 원고에 인라인 SVG로 포함돼 있던 다이어그램 4장(GT 보정
    사례, 가려짐 반달 모양, histogram matching 단계, 축 대조 검증)도 같은 폴더에
    SVG 그대로 옮겼다 — 색·구성은 원고 그대로이고 별도로 다시 그리지 않았다.
  */
  {
    ...
    teamSize: "AI Vision 그룹 · 산부인과 AI 랩 · 2D Follicle™ 팀",
    summary:
      "하복부 초음파 다중 구조물 검출 과제가 중단된 뒤, 다른 사수의 팀으로 옮겨 부인과 초음파 AI 기능 개발에 참여했습니다. 2D Follicle™ 팀에서 난포 세그멘테이션 성능을 올리는 것을 목표로, 학습 데이터 구축·모델 개선 실험·측정 기능(C++) 개발 세 갈래로 참여했습니다.",
    summaryMore: [
      "모델 구조를 바꾸는 대신 학습 데이터를 새로 만들고 다듬어 성능을 올리는 쪽을 택했습니다. 수천 장 규모의 초음파 영상을 선별해 GT를 작성하고 기존 GT의 품질을 보정했으며, 한 차례는 2,534장 중 883장을 선별했습니다. 이렇게 만든 데이터가 지금 제품에 탑재된 난포 검출 모델의 학습 데이터로 쓰이고 있습니다. 제가 학습시킨 모델 자체는 채택되지 않았지만, 성능을 올리기 위해 만든 데이터는 남아 지금도 쓰이고 있습니다.",
    ],
    ...
    overview: [
      {
        label: "소속",
        value:
          "삼성메디슨 AI Vision 그룹(현 AI&Informatics 그룹) 산부인과 AI 랩 · 2D Follicle™ 팀",
      },
      {
        label: "제품",
        value:
          "2D Follicle™ — 여러 개의 난포 크기를 한 화면에서 자동 측정 (적용 장비: V8 · V7 · V6 · HS60 · HS50 · HS40 · HM70 EVO)",
      },
      {
        label: "목표",
        value:
          "난소 초음파에서 여러 난포를 구분해내는 2D Follicle™의 세그멘테이션 성능 향상",
      },
      {
        label: "담당",
        value: "학습 데이터 구축 · 모델 개선 실험 · 측정 기능(C++) 개발",
      },
      {
        label: "데이터 규모",
        value:
          "수천 장 규모 초음파 영상 선별 및 GT 작성·보정 — 한 차례 2,534장 중 883장 선별",
      },
      {
        label: "제품에 남은 것",
    ...
    overviewFigures: [
      {
        src: "/samsung-medison-detection/follicle-segmentation-result.png",
        width: 800,
        height: 522,
        alt: "난소 초음파 화면에서 난포 4개가 번호와 윤곽선으로 세그멘테이션된 결과 화면",
        caption: "이런 세그멘테이션의 성능을 높이는 것이 이 과제의 목표였다",
        captionEmphasis: true,
      },
    ],
    myRole:
      "4개월간 2D Follicle™ 세그멘테이션 성능 향상(데이터 구축·모델 실험)과 AddCaliper(C++) 개발, 두 갈래로 참여했습니다.",
    myRoleAreas: [
      {
        title: "2D Follicle™",
      ...
      },
      {
        ...
        desc: "검사자가 찍은 네 점에서 난포 윤곽선을 생성하는 스플라인 보간 로직 개발",
      },
    ],
    featureGroups: [
      {
        label: "2D Follicle™",
        intro:
          "2D Follicle™은 부인과 검사 시, 여러 개의 난포 크기를 측정하고 상태에 대한 정보를 제공하는 기능입니다.",
        items: [
      {
        name: "1. 학습 데이터 구축",
        tagline:
          "모델 구조를 바꾸는 대신 학습 데이터를 늘리고 다듬어 성능을 올리는 접근을 택했습니다. 그렇게 만든 데이터가 지금 제품에 탑재된 난포 검출 모델의 학습 데이터입니다. 수집 경로는 한 가지가 아니었습니다.",
        metric: "2,534장 중 883장 선별",
        problemLabel: "장비 화면을 그대로 쓸 수 없던 이유",
        problem:
          "장비 화면에는 이미 측정 결과가 그려져 있습니다. 노란 캘리퍼 마커와 수치, 자동 검출된 윤곽선이 영상 위에 덧씌워진 상태라, 그대로 저장하면 모델이 조직이 아니라 마커를 보고 학습하게 됩니다.",
        solutionLabel: "세 갈래로 모았다",
        solutionBlocks: [
          {
            title: "A. 장비에서 직접 캡처",
            figure: {
              src: "/samsung-medison-detection/ovary-raw-capture.png",
              width: 800,
              height: 522,
              alt: "캘리퍼 마커나 윤곽선 없이 오버레이를 끈 상태로 캡처한 난소 초음파 화면 두 장",
              caption: "수집 대상 화면의 예 — 측정 오버레이를 끈 상태로 캡처했다",
            },
            points: [
              "장비에 저장된 기록에서 측정 표시를 끄는 기능을 찾아, 오버레이가 없는 상태의 영상만 캡처해 수집",
              "파일명과 폴더명도 규칙을 정해 정리",
            ],
          },
          {
            title: "B. 병원 원본에서 난포만 골라내기",
            points: [
              "병원에서 여러 차례에 걸쳐 받은 새 초음파 영상은 한 영상 안에 난포·자궁·태아가 섞여 있어, 학습에 쓸 난포 영상만 골라내는 선별을 매번 반복",
              "임신 상태인 자궁, 난포 없이 속이 불투명하거나 출혈이 고인 것으로 보이는 상, 자궁을 보는 뷰라 난포가 너무 작게 잡힌 경우는 제외",
              "한 차례는 2,534장 중 883장을 선별 — 고른 영상에는 GT를 새로 그려 학습 데이터로 만듦",
              "어떤 상이 난포이고 경계를 어디로 볼지는 혼자 판단할 수 없어 임상의와 계속 상의하며 진행 — 그 과정에서 초음파 영상을 해석하는 법과 관련 임상 지식을 자연스럽게 익힘",
            ],
          },
          {
            title: "C. 다른 제품의 데이터까지 활용",
            figure: {
              src: "/samsung-medison-detection/volume-5d-follicle.png",
              width: 800,
              height: 522,
              alt: "5D Follicle™ 볼륨 기반 초음파 화면에서 여러 난포가 색상별 윤곽으로 표시된 화면",
              caption: "볼륨 기반 화면 — 여기서도 2D 학습에 쓸 데이터가 나온다",
            },
            points: [
              "5D Follicle™은 별개 제품이지만 그 검사에서 나오는 데이터에도 2D Follicle™ 학습에 쓸 수 있는 것이 있어 전부 활용",
              "난포가 찍혀 있을 수 있는 다른 장비·검사의 데이터도 빠짐없이 열어보고, 그중 학습에 쓸 수 있는 상인지는 임상의와 함께 확인하며 수집",
            ],
          },
          {
            title: "레이블링과 품질 보정",
            figure: {
              src: "/samsung-medison-detection/gt-correction-cases.svg",
              width: 1360,
              height: 900,
              alt: "넓게 칠해진 GT를 실제 경계에 맞게 보정하고, 겹쳐서 하나로 칠해진 두 난포를 앞쪽 경계선을 따라 지워 원과 반달로 분리하는 두 가지 보정 사례",
              caption: "넓게 칠해진 경계는 다시 그리고, 겹친 난포는 앞쪽 경계선을 따라 지워 나눈다",
            },
            points: [
              "수집한 데이터는 임상의와 함께 레이블을 작성하고, 기존 데이터의 품질도 보정",
              "레이블링 효율을 높이기 위해 노이즈 제거를 비롯한 영상처리로 레이블 초안을 자동 생성하는 방식도 구현",
              "실제 난포보다 넓게 칠해져 경계가 부정확한 GT는 경계를 실제 크기에 맞게 다시 그림",
              "겹쳐 있는 두 난포가 하나의 마스크로 칠해진 경우는 앞쪽 난포의 경계선을 따라 선을 지워 두 영역으로 분리 — 뒤쪽 난포에는 가려지지 않은 반달 부분만 남김",
            ],
            note: "반대 방향의 문제도 있었다 — 눈으로 보면 분명히 하나인 난포인데 GT에는 여러 조각으로 나뉜 경우는 morphology 연산으로 끊긴 영역을 이어 하나로 합쳤다. 나누는 일과 합치는 일 모두 실제 난포의 모양에 맞추기 위한 작업이었다.",
          },
        ],
      },
      {
        name: "2. 모델 개선 실험",
        tagline:
          "이 실험의 목표는 새 모델을 찾는 것이 아니라 기존 2D Follicle™의 성능을 끌어올리는 것이었습니다. 구조는 그대로 두고, 손실 함수·Augmentation·학습률 스케줄·학습 데이터 자체에서 바꿀 수 있는 것을 찾았습니다.",
        metric: "손실 함수 3방향 비교 → FN 가중 채택",
        problemLabel: "EDA로 짚은 것",
        problemList: [
          "가려짐으로 남는 반달 모양 — 난포 두 개가 나란히 있어도 초음파 상에서 하나가 다른 하나에 가려지면 GT는 보이는 부분만 표시할 수밖에 없다. 잘못된 라벨링이 아니라 가려짐이 있는 한 피할 수 없는 경우",
          "장비별 배경색 차이 — 검은 배경과 흰 배경이 섞여 있어 같은 조직이라도 모델이 다르게 반응할 위험",
          "출처별 밝기 분포 차이 — 같은 조직이 장비마다 더 밝거나 더 어둡게 찍힘",
          "크기별 성능 격차 가능성 — 모델이 특정 크기대에서 유독 약한지 확인 필요",
        ],
        solutionLabel: "처리 방식",
        solutionBlocks: [
          {
            title: "가려짐은 오류가 아니다",
            figure: {
              src: "/samsung-medison-detection/occlusion-crescent.svg",
              width: 1360,
              height: 560,
              alt: "실제로는 온전한 원인 난포 두 개가 초음파에서 하나가 다른 하나에 가려 GT에는 반달 모양으로만 표시되는 과정",
              caption: "실제로는 온전한 원이지만, 가려진 부분은 알 수 없어 GT는 보이는 만큼만 남는다",
            },
            points: [],
          },
          {
            title: "배경색 통일 — 조건을 좁혀 안전하게",
            points: [
              "장수가 많아 손으로 할 수 없어, 영상처리로 배경 영역을 인식해 자동으로 바꾸는 함수를 만듦",
              "밝기만으로 판정하면 신호가 거의 없어 어둡게 나오는 조직 영역까지 배경으로 잘못 잡혀 실제 조직 정보가 지워지는 문제가 있었음",
              "흰 배경 영상은 특정 한 장비에서만 나왔고 그 장비는 화면 구성이 고정이라 배경이 차지하는 위치가 항상 같음을 확인 — 배경 영역의 고정 틀을 만들어 그 부분만 0으로 채우는 방식으로 전환. 일반해를 찾다가 조직을 지우는 것보다 범위를 한정해 확실하게 처리하는 편을 택함",
            ],
          },
          {
            title: "출처별 밝기 분포 — histogram matching",
            figure: {
              src: "/samsung-medison-detection/histogram-matching.svg",
              width: 1360,
              height: 590,
              alt: "기준 데이터의 난포 영역 밝기 분포를 추출해 다른 쪽 데이터의 해당 픽셀을 어둡게 맞추고, 기준 원본과 합친 조합도 실험하는 세 단계",
              caption: "기준 데이터의 난포 영역 분포를 뽑아 다른 쪽 데이터를 맞추고, 원본과 합친 조합도 함께 실험",
            },
            points: [
              "CLAHE 같은 대비 조정은 커널 단위 국소 연산이라 제외 영역 경계 근처만 달라지고 전체 밝기 성질은 그대로였음",
              "GT 마스크로 난포 영역만 어둡게 만드는 방식은 접었다 — 추론 시점에는 GT가 없어 같은 변환을 재현할 수 없고, 모델이 인위적으로 만들어진 어두움을 단서로 삼을 위험도 있었음. 정답을 입력에 섞지 않는다는 원칙을 세움",
              "원인은 국소 대비가 아니라 출처별 밝기 분포 자체의 차이 — 한쪽 데이터로 다른 쪽을 보정했을 때 성능이 오른 것이 근거",
              "난포 영역의 분포만 뽑아 기준으로 삼고 다른 쪽의 해당 픽셀을 어둡게 눌러 맞추는 방식도 실험 — 어느 쪽을 기준으로 할지는 양방향 모두 시도해 비교했고, 기준이 된 원본과 합친 조합도 함께 실험",
            ],
          },
          {
            title: "크기별 성능 격차 — S/M/L 분류",
            points: [
              "학습 데이터의 난포 장축을 모두 구해 히스토그램을 만들고, 분포 기준 S·M·L 세 구간으로 나눠 크기별 모델 성능을 따로 확인",
              "장축 산출에는 cv2.fitEllipse 대신 cv2.minEnclosingCircle 채택 — 난포는 실제로는 구형이라 경계에서 튀어나온 부분도 노이즈가 아니라 그 방향의 실제 반지름이므로, 여러 방향을 평균 내는 대신 경계 위 두 점 사이 최대 거리를 기준으로 삼음",
            ],
          },
          {
            title: "베이스라인과 튜닝",
            points: [
              "새 모델을 고르는 대신 기존에 쓰던 세그멘테이션 모델을 그대로 베이스라인으로 삼고, 그 위에서 바꿀 수 있는 것들을 찾음 — 적용할 기법은 최신 논문을 조사해 선정",
              "Augmentation — 회전, 이동·확대축소, Gaussian noise, 수직 뒤집기, zoom in",
              "학습률 스케줄러 CosineAnnealingWarmRestarts — 학습률을 코사인 곡선을 따라 낮추다 주기적으로 다시 올려 한 지점에 갇히지 않고 더 나은 해를 탐색하도록 파라미터를 조정해 사용",
              "zoom in 여백 — 난포를 모두 담도록 자른 영역은 원본마다 가로세로 비율이 달라, 목표 크기로 늘리면 비율이 깨지고 비율을 지키면 여백이 생김. 난포의 형태 자체가 판단 근거이므로 비율을 지키고 여백을 넣는 쪽을 택했고, 어느 쪽이 나은지는 짐작 대신 두 방식으로 데이터셋을 각각 만들어 비교하도록 준비",
              "여백은 0(검은색)으로 채움 — 함께 학습하는 다른 출처 데이터의 바깥 영역이 이미 0이라, 여백을 0으로 두는 편이 두 데이터의 성질을 맞추는 데 유리했음",
              "데이터는 원본·변환본·조합으로 버전을 나눠 관리하고 검증 세트도 처리 여부·출처 조합에 따라 나눠 각각 비교 — 어떤 실험에 어떤 학습·검증 데이터를 썼는지 그때그때 기록하는 것을 규칙으로 삼음",
            ],
          },
        ],
        troublesLabel: "손실 함수 실험과 평가",
        troubles: [
          {
            title: "손실 함수 비교 — 어느 쪽 오류에 벌점을 줄 것인가",
            table: {
              head: ["구성", "설계", "결과"],
              rows: [
                [
                  "Dice loss 단독",
                  "BCE를 빼고 Dice만 사용 — 비교의 기준선",
                  "—",
                ],
                [
                  "Tversky Loss (FN 가중 0.7 / FP 0.3)",
                  "TI = TP / (TP + αFN + βFP), 손실 = 1 − TI. 놓치는 쪽(FN)에 더 큰 벌점",
                  "가장 좋은 결과 — 채택",
                ],
                [
                  "Tversky Loss (FP 가중)",
                  "확신 있는 픽셀만 난포로 표시하도록 유도",
                  "실제 난포 영역까지 놓쳐 FN 증가, 경계가 흐린 난포에서 지나치게 보수적인 예측 → 노이즈 있는 검증 데이터에서 성능 저하",
                ],
                [
                  "Focal Tversky Loss",
                  "Tversky에 gamma를 더해 쉬운 샘플 비중을 낮추고 어려운 샘플에 집중",
                  "—",
                ],
              ],
            },
            effect:
              "난포 검출에서는 있는 난포를 놓치는 쪽이 검사자에게 더 곤란하고, 잘못 잡은 것은 눈으로 걸러낼 수 있다고 판단해 FN에 0.7, FP에 0.3의 가중치를 둔 구성을 택했다. 짐작이 아니라 가중치 없는 원본과 반대 가중치까지 모두 비교해 확인한 결과이며, 이 결론은 아래 개수 기반 평가 절차로 다시 검증했다.",
            tags: ["Tversky Loss", "손실 함수"],
            tech: ["PyTorch"],
          },
          {
            title: "개수를 세는 평가 — 손실 값은 작은 난포를 놓쳐도 티가 안 난다",
            problem:
              "세그멘테이션 손실은 픽셀 단위로 계산되는데, 실제로 알아야 하는 것은 난포를 몇 개 찾았고 몇 개를 놓쳤는지다. 작은 난포는 전체 픽셀의 극히 일부라 하나를 통째로 놓쳐도 손실 점수는 거의 떨어지지 않는다.",
            solution:
              "GT와 예측 각각에서 연결된 영역에 BBox를 씌우고 양쪽을 짝지어 대조했다. 짝이 없는 GT 사각형은 놓친 것(FN), 짝이 없는 예측 사각형은 잘못 잡은 것(FP)으로 셌다. 경계의 정밀도가 아니라 개수가 목적이라 사각형 겹침만으로 충분했고, 사각형에서 나오는 크기로 크기대별 누락 여부도 함께 확인했다.",
            effect:
              "손실 함수 실험에서 FN 가중이 가장 좋았다는 결론도 이 평가 절차로 얻은 것이다.",
            tags: ["평가 지표", "BBox"],
          },
          {
            title: "ONNX 변환 검증",
            problem:
              "학습한 모델을 장비에 올리려면 ONNX로 변환해야 하는데, 변환 과정에서 결과가 달라지면 그때까지의 평가가 의미를 잃는다.",
            solution:
              "변환 전(PyTorch pth)과 변환 후 출력이 같은지 대조하는 작업을 병행했다.",
            effect:
              "제품에 최종 탑재된 것은 같은 팀 선임이 개발한 모델이고, 이 기간에 만든 모델은 채택되지 않았다. 다만 채택된 모델이 학습하는 데이터는 이 기간에 만든 것이다.",
            tags: ["ONNX", "배포 검증"],
          },
        ],
      },
      ...
      {
        label: "AddCaliper — 난포 윤곽선 생성 (C++)",
        items: [
      {
        name: "스플라인 보간으로 윤곽선 생성",
        when: "2024.07.15 – 2024.08.31",
        tagline:
          "2D Follicle™에는 검사자가 직접 난포를 지정해 측정하는 AddCaliper 도구가 있습니다. 여기서 검사자가 찍은 네 점에서 난포 윤곽선을 그려내는 계산을 개발했습니다. 타원으로는 난포 모양을 그대로 담지 못해, 네 점을 지나는 곡선을 새로 그리는 방식으로 만들었습니다.",
        figures: [
          {
            src: "/samsung-medison-detection/addcaliper-four-points.png",
            width: 1360,
            height: 600,
            alt: "트랙볼로 찍은 장축·단축 네 점에서 닫힌 곡선을 보간해 면적과 둘레를 산출하는 세 단계",
            caption: "네 점 입력에서 측정값이 나오기까지 — 가운데 곡선 생성이 담당 범위",
          },
        ],
        problemLabel: "왜 타원이 아닌가",
        problem:
          "검사자가 트랙볼로 난포의 장축과 단축 끝점 네 개를 찍으면 네 점을 모두 지나는 닫힌 곡선이 그려지고, 그 곡선을 기준으로 크기가 산출됩니다. 두 축은 서로 교차하기만 하면 되고 직교일 필요는 없습니다 — 검사자는 수직으로 찍는다고 생각하지만 실제로는 조금씩 틀어지기 때문입니다. 타원은 두 축에 대해 대칭이라 한쪽만 볼록한 난포 경계를 따라가지 못합니다. 실제 난포는 인접한 난포에 눌려 비대칭인 경우가 많고, 타원으로 재면 그만큼 면적에 오차가 생깁니다.",
        solutionLabel: "구현",
        solutionBlocks: [
          {
            title: "네 점을 지나는 스플라인",
            figure: {
              src: "/samsung-medison-detection/ellipse-vs-spline.png",
              width: 1360,
              height: 640,
              alt: "타원 피팅은 경계와 어긋나는 구간이 남지만 스플라인 보간은 사분면마다 곡률이 달라 경계를 따라간다",
              caption: "왼쪽 타원은 어긋나고, 오른쪽 스플라인은 경계를 따라간다",
            },
            points: [
              "네 점을 지나는 스플라인은 구간마다 다항식이 따로 계산되므로 사분면별로 곡률이 달라져 비대칭 경계를 따라감",
              "닫힌 곡선은 하나의 x에 y가 둘 대응해 y = f(x)로 표현되지 않음 — 곡선을 따라간 누적 거리를 매개변수로 삼아 x, y를 각각 그 매개변수의 함수로 분리해 두 개의 스플라인으로 풀었음",
              "외부 라이브러리를 반입할 수 없는 환경이라 자연 3차 스플라인을 직접 구현",
            ],
          },
          {
            title: "예외 처리",
            figure: {
              src: "/samsung-medison-detection/axis-validation.png",
              width: 1360,
              height: 600,
              alt: "두 선분이 교차하면 통과, 빗나가거나 일직선이면 CCW 부호로 거부하는 판정 흐름",
              caption: "장축을 기준으로 단축의 두 끝점이 반대편이면 통과, 같은 편이거나 한 직선 위면 거부",
            },
            points: [
              "검사자가 찍은 두 축이 실제로 교차하는지 먼저 검사 — 세 점의 방향 관계(CCW, Counter-ClockWise)로 두 선분의 위치 관계를 판정하고, 두 선분 각각을 기준으로 양쪽 모두 확인",
              "축이 빗나갔거나 네 점이 한 직선 위에 놓인 입력은 곡선이 만들어지지 않으므로 여기서 걸러내고 오류를 반환",
            ],
          },
          {
            title: "검증",
            figure: {
              src: "/samsung-medison-detection/axis-verification.svg",
              width: 1360,
              height: 600,
              alt: "입력한 두 축과 결과 마스크에서 역산한 축을 겹쳐 대조해 크게 벗어나거나 꼬인 경우만 걸러낸다",
              caption: "입력한 축(주황)과 결과에서 역산한 축(청록)은 애초에 정확히 겹칠 수 없다",
            },
            points: [
              "사람이 직접 점을 찍어야 동작하는 기능이라 자동 검증이 까다로웠음",
              "결과 마스크에서 외접 사각형과 장·단축을 역산해 입력한 축과 대조하는 방식으로 확인",
              "외접 사각형은 화면 축에 정렬되고 두 축을 직교로 강제하는 반면 실제 입력은 기울어져 있고 직교도 아니므로, 일치 여부가 아니라 곡선이 크게 벗어나거나 꼬이는 경우를 잡아내는 용도로 사용",
            ],
          },
          {
            title: "장비에 넣기",
            points: [
              "알고리즘은 파이썬으로 먼저 구현해 곡선이 의도대로 그려지는지 확인한 뒤 C++로 이식",
              "AddCaliper에 연결해 장비 소프트웨어에 빌드하고, 사내 테스트 빌드에서 실제 장비 동작까지 확인",
            ],
          },
        ],
      },
    ...
    closingLabel: "마무리",
    closing: [
      "성능을 올리기 위해 데이터를 만들고, 모델을 실험하고, 장비에 들어가는 측정 기능을 개발했습니다. 세 가지를 했지만 무게중심은 첫 번째에 있었습니다.",
      "앞선 프로젝트(하복부 초음파 다중 구조물 검출)는 데이터 품질이 병목이라는 결론과 함께 중단됐습니다. 그다음에 맡은 일이 데이터를 직접 만드는 쪽이었던 것은 우연이 아니었다고 봅니다. 모델 구조를 바꿔서 얻는 개선과 데이터가 깨끗해져서 얻는 개선은 크기가 다릅니다. 오버레이가 찍힌 영상 한 장이 섞이면 그 뒤의 모든 실험이 의심스러워집니다.",
      "그래서 이 기간의 실험은 모델을 갈아끼우는 방식이 아니었습니다. 쓰던 모델은 그대로 두고, 데이터를 새로 모으고 잘못된 레이블을 고치고 출처가 다른 영상의 성질을 맞추는 쪽으로 성능을 올리려 했습니다. 손실 함수나 Augmentation을 바꾼 것도 결국 같은 데이터를 어떻게 더 잘 쓸 것인가의 문제였습니다.",
      "혼자 판단할 수 없는 일이 많았습니다. 어떤 상이 학습에 쓸 수 있는 난포인지, 경계를 어디로 봐야 하는지는 매번 임상의와 확인하며 정했습니다. 레이블의 기준이 정해지지 않으면 모델이 무엇을 학습하는지도 정해지지 않습니다. 도메인 전문가와 그 기준을 맞춰 고정하는 일이 데이터 작업의 절반이었습니다.",
      "판단이 필요할 때는 근거를 먼저 만들어두려 했습니다. 여백을 넣을지 비율을 깨뜨릴지 짐작으로 정하는 대신 두 가지 데이터셋을 만들어 비교할 수 있게 했고, 손실 값만으로는 작은 난포를 놓친 것이 보이지 않아 난포 하나를 단위로 세는 평가 절차를 따로 만들었습니다. 반대로 하지 않기로 한 것도 있습니다. GT를 써서 입력 영상을 손대면 원하는 결과를 만들 수는 있지만, 추론 시점에는 없는 정보라 쓰면 안 되는 방법이었습니다.",
      "결과만 놓고 보면 제가 만든 모델은 채택되지 않았고, 측정 기능은 사내 테스트 빌드까지 갔습니다. 다만 제품에 탑재된 모델이 학습하는 데이터는 이 기간에 제가 만든 것입니다.",
      "지표가 무엇을 못 보고 있는지 먼저 확인할 것, 정답을 입력에 섞지 않을 것. 이 두 가지가 이 4개월에서 가장 크게 남은 것입니다.",
    ],
  },
];
